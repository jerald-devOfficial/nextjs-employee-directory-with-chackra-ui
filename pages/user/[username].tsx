...
interface EachEmployee {
  user: Employee;
}

export const getStaticProps: GetStaticProps<EachEmployee, Params> = async (
  context
) => {
...
const UserPage: NextPage<EachEmployee> = ({ user }): JSX.Element => {
  return (
      ...
      <Box>
        <Box position="relative">
          <Image
            ...
            height="250px"
            objectFit="cover"
            alt={`${user.first_name}'s cover photo`}
          />
          <Flex
            alignItems="flex-end"
            position="absolute"
